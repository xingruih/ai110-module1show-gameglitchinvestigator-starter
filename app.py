...
# FIX: Changed initial attempts from 1 to 0 so the count is accurate from the start using Claude Code
...
# FIX: Wrapped input and submit in st.form so the guess value submits on first click using Claude Code
...
# FIX: New Game now resets all state (score, status, history) and uses difficulty range using Claude Code
if new_game:
    st.session_state.attempts = 0
    st.session_state.score = 0
    st.session_state.status = "playing"
